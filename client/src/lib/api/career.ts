...
export const CAREER_QUERY_KEY = ["career-section"] as const;
...
export const normalizeCareerSection = (value: unknown): CareerSectionData => {
  ...
  return {
    ...
    highlightCards: normalizeHighlightCards(source.highlightCards),
...
export const normalizeCareerList = (value: string[] | string) => {
  const source = Array.isArray(value) ? value : value.split(/\r?\n/);
  return uniqueTextList(source, []);
};

export async function fetchCareerSection() {
  if (!API_BASE) return fallbackCareerSection;

  const res = await fetch(`${API_BASE}/career-section`, { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch career section");
