...
const normalizeImages = (images: unknown): HeroImage[] => {
  ...
  const cleaned = images
    .map((img) => {
      ...
      return {
        _id:
          typeof (img as HeroImage)._id === "string"
            ? (img as HeroImage)._id
            : undefined,
...
const normalizeButtons = (buttons: unknown): HeroButton[] => {
  ...
  const cleaned = buttons
    .map((btn) => {
      ...
      return {
        _id:
          typeof (btn as HeroButton)._id === "string"
            ? (btn as HeroButton)._id
            : undefined,
        label,
        href,
        style:
          style === "secondary" || style === "outline" || style === "ghost"
            ? style
            : "primary",
...
const normalizeStats = (stats: unknown): HeroStat[] => {
  ...
  const cleaned = stats
    .map((stat) => {
      ...
      return {
        _id:
          typeof (stat as HeroStat)._id === "string"
            ? (stat as HeroStat)._id
            : undefined,
        label,
        value,
...
export const fallbackHero: HeroSectionData = {
  ...
  images: [
    { url: images.Hero_bg1, alt: "Hero background 1", order: 0 },
    { url: images.Hero_bg2, alt: "Hero background 2", order: 1 },
    { url: images.Hero_bg3, alt: "Hero background 3", order: 2 },
    { url: images.Hero_bg4, alt: "Hero background 4", order: 3 },
    { url: images.Hero_bg5, alt: "Hero background 5", order: 4 },
  ],
  scrollText: "Scroll",
  showScrollIndicator: true,
};

export const normalizeHeroSection = (
  hero?: Partial<HeroSectionData> | null,
): HeroSectionData => ({
